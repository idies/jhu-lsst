...
def dbconnect(user='lsst', password='lsst2018', host='127.0.0.1', database='vdc1', raise_on_warnings=True):
...
def export_by_exposure_id(tablename, csvpath, cnx):
    ...
    for row in results:
        ...
        sql, filename = generateSourceSql(getChunkIDFromTablename(tablename), tablename, getFileNamefromTablename(tablename, csvpath), expid)
...
def generateObjectFullOverlapSql(chunkid, tablename, filename):
...
def generateForcedSourceSql(chunkid, tablename, filename, dbname):
...
def generateSourceSql(chunkid, tablename, filename, exposure_id=None):
    ...
    from vdc1.{tablename} {where};""".format(chunkid=chunkid,filename=filename, tablename=tablename, where=where)

    return source_sql, filename
