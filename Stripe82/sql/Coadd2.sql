...
GO
--
alter table test.coadd
add constraint pk_coadd_cobjid
primary key clustered (cobjid)
on [COADD]

----------
dbcc traceon(610)
INSERT test.coadd with (tablock)
SELECT
...
modify filegroup [COADD] default
go
...
------------------------------------------
DECLARE @zoneHeight float = cast(10.0 as float)/3600.0, @theta float = cast(1 as float)/3600;
DECLARE @ra_lo float = -180, @ra_hi float = 180;
...
----------------------------------------------
DECLARE @theta float;
SELECT @theta = value FROM test.Pmts
where name = 'theta'
...
CREATE TABLE test.match (
 ...
 PRIMARY KEY clustered(cobjid, objid)
)
GO
--
dbcc traceon(610)

INSERT test.match with (tablock)
SELECT
 c.cobjid,
 p.objid,
 n.distance,
